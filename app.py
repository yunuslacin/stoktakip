...
def stock_requests():
    products = Product.query.order_by(Product.name).all()
    projects = Project.query.order_by(Project.name).all()
    min_due_date = (local_today() + datetime.timedelta(days=1)).isoformat()
    if request.method == "POST":
        product_ids = request.form.getlist("product_id[]")
        quantities = request.form.getlist("quantity[]")
        notes = request.form.get("notes", "").strip()
        ...
        for idx, product_raw in enumerate(product_ids):
            ...
            request_items.append(
                (product_id, quantity, project_id, due_date_value, project.manager_id)
            )
        ...
        db.session.commit()
        flash(
            f"{order_number} sipariş numaralı talebiniz ilgili proje yöneticilerine gönderildi",
            "success",
        )
        return redirect(url_for("stock_requests"))
    my_requests_query = (
    ...
    if g.user.is_manager:
        approvals_query = (
            ...
            .filter_by(manager_id=g.user.id, status="pending")
            .order_by(StockRequest.created_at.asc())
        )
...
def approve_request(request_id: int):
    ...
    stock_request.approved_at = datetime.datetime.utcnow()
...
def export_request_pdf(group_key: str):
    ...
    for idx, line in enumerate(group["lines"], start=1):
        ...
        approval_display = format_local_datetime(line.get("approved_at")) if line.get("approved_at") else "-"
        line_data.append(
            [
                ...
                Paragraph(pdf_safe(approver_name), info_style),
            ]
        )

    lines_table = Table(
        line_data,
        colWidths=[30, 140, 55, 90, 60, 60, 60, 60],
        hAlign="LEFT",
    )
