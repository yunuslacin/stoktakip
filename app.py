...
def export_request_pdf(group_key: str):
    ...
    def ensure_space(amount: float) -> None:
        nonlocal y_position
        if y_position <= amount:
            pdf.showPage()
            y_position = height - 40

    def draw_text(text: str, *, font: str = "Helvetica", size: int = 10, spacing: float = 14) -> None:
        nonlocal y_position
        ensure_space(60)
    ...
    pdf.drawString(40, y_position, f"Talep Özeti - {group['display_number']}")
    y_position -= 24
    ...
    draw_text(f"Oluşturma: {created_at}")
    draw_text(f"Talep Eden: {requester}")
    draw_text(f"Yönetici: {manager}")
    draw_text(f"Durum: {group['status_label']}")
    if group.get("notes"):
        draw_text(f"Not: {group['notes']}")

    draw_text("Satırlar:", font="Helvetica-Bold", size=12, spacing=18)
    for idx, line in enumerate(group["lines"], start=1):
        product_name = line["product"].name if line.get("product") else "-"
        due_text = (
            line["due_date"].strftime("%d.%m.%Y") if line.get("due_date") else "Belirtilmedi"
        )
        project_name = line["project"].name if line.get("project") else "-"
        draw_text(
            f"{idx}. Ürün: {product_name} | Miktar: {line['quantity']} | Proje: {project_name}",
            spacing=16,
        )
        approver = line.get("approver")
        approval_info = (
            f" | Onaylayan: {approver.username}" if approver else ""
        )
        draw_text(
            f"   Termin: {due_text} | Satır Durumu: {line['status']}{approval_info}",
            spacing=18,
        )

    pdf.setFont("Helvetica", 9)
    pdf.drawString(40, 30, "Stok Takip Sistemi")
    pdf.save()
    buffer.seek(0)
