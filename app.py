...
app.config.setdefault("SMTP_PORT", int(smtp_port_env) if smtp_port_env else 587)
...
if smtp_use_tls_env is None:
    app.config.setdefault("SMTP_USE_TLS", True)
else:
    app.config.setdefault(
        "SMTP_USE_TLS", smtp_use_tls_env.lower() in {"1", "true", "yes", "on"}
    )
app.config.setdefault("MAIL_SENDER", os.environ.get("MAIL_SENDER"))
smtp_timeout_env = os.environ.get("SMTP_TIMEOUT")
app.config.setdefault("SMTP_TIMEOUT", int(smtp_timeout_env) if smtp_timeout_env else 10)
...
DEFAULT_AUTO_EMAIL_SUBJECT = "[Stok Talebi] {order_number} - {stage_label}"
DEFAULT_AUTO_EMAIL_BODY = (
    "Merhaba {approver_name},\n\n"
    "{requester_name} kullanıcısı {line_count} satırlı yeni stok talebi oluşturdu.\n"
    "Sipariş numarası: {order_number}\n"
    "Onay aşaması: {stage_label}\n"
    "Projeler: {projects_text}\n"
    "Termin tarihleri: {due_dates_text}\n\n"
    "Satırlar:\n{lines_text}\n\n"
    "Talep notu: {notes}\n\n"
    "Talebe erişim bağlantısı: {link}\n"
)

AUTO_EMAIL_PLACEHOLDERS = [
    ("{order_number}", "Talebin sipariş numarası"),
    ("{stage_label}", "E-postanın gönderildiği onay aşaması"),
    ("{requester_name}", "Talebi oluşturan kullanıcının adı"),
    ("{requester_email}", "Talebi oluşturan kullanıcının e-posta adresi"),
    ("{approver_name}", "E-postanın ulaştığı onaycının adı"),
    ("{projects_text}", "Talepte yer alan proje adları"),
    ("{due_dates_text}", "Talep satırlarının termin tarihleri"),
    ("{line_count}", "Talepteki satır sayısı"),
    ("{notes}", "Talebe eklenen genel not"),
    ("{lines_text}", "Her satırın özet bilgileri"),
    ("{link}", "Talebe ait uygulama bağlantısı"),
]
...
def send_email_notification(subject: str, body: str, recipients: list[str]) -> bool:
    filtered_recipients = [recipient for recipient in recipients if recipient]
    if not filtered_recipients:
        log_email_notification(subject, filtered_recipients, body, False, "Alıcı bulunamadı")
        return False
    server = app.config.get("SMTP_SERVER")
    if not server:
        message = "SMTP_SERVER tanımlı değil, e-posta gönderimi atlandı"
        app.logger.info(message)
        log_email_notification(subject, filtered_recipients, body, False, message)
        return False
    port = int(app.config.get("SMTP_PORT", 587))
    username = app.config.get("SMTP_USERNAME")
    password = app.config.get("SMTP_PASSWORD")
    sender = app.config.get("MAIL_SENDER") or username or "no-reply@example.com"
    use_tls = bool(app.config.get("SMTP_USE_TLS", True))
    timeout = int(app.config.get("SMTP_TIMEOUT", 10))

    message = EmailMessage()
    message["Subject"] = subject
    message["From"] = sender
    message["To"] = ", ".join(filtered_recipients)
    ...
    try:
        ...
        log_email_notification(subject, filtered_recipients, body, True)
        return True
    except Exception as exc:  # pragma: no cover - log and continue
        error_message = str(exc)
        app.logger.exception("E-posta gönderimi başarısız oldu")
        log_email_notification(subject, filtered_recipients, body, False, error_message)
        return False
...
class EmailConfig(db.Model):
    ...
    smtp_timeout = db.Column(db.Integer, default=10)
    auto_notify_enabled = db.Column(db.Boolean, default=True)
    auto_subject = db.Column(db.String(255), nullable=True)
    auto_body = db.Column(db.Text, nullable=True)
...
def ensure_schema_upgrades() -> None:
    ...
    add_column("email_config", "auto_notify_enabled", "BOOLEAN")
    add_column("email_config", "auto_subject", "VARCHAR(255)")
    add_column("email_config", "auto_body", "TEXT")
    ...
    if not inspector.has_table("email_config"):
        EmailConfig.__table__.create(db.engine)
...
def get_or_create_email_config() -> EmailConfig:
    ...
    if config is None:
        config = EmailConfig(
            ...
            smtp_timeout=app.config.get("SMTP_TIMEOUT"),
            auto_notify_enabled=True,
            auto_subject=DEFAULT_AUTO_EMAIL_SUBJECT,
            auto_body=DEFAULT_AUTO_EMAIL_BODY,
        )
        db.session.add(config)
        db.session.commit()
    updated = False
    if config.auto_subject is None:
        config.auto_subject = DEFAULT_AUTO_EMAIL_SUBJECT
        updated = True
    if config.auto_body is None:
        config.auto_body = DEFAULT_AUTO_EMAIL_BODY
        updated = True
    if config.auto_notify_enabled is None:
...
def apply_email_config(config: EmailConfig | None = None) -> None:
    if config is None:
        config = EmailConfig.query.first()
    if not config:
        return
    app.config["SMTP_SERVER"] = config.smtp_server
    app.config["SMTP_PORT"] = config.smtp_port or 587
    app.config["SMTP_USERNAME"] = config.smtp_username
    app.config["SMTP_PASSWORD"] = config.smtp_password
    app.config["SMTP_USE_TLS"] = True if config.smtp_use_tls is None else config.smtp_use_tls
    app.config["MAIL_SENDER"] = config.mail_sender
    app.config["SMTP_TIMEOUT"] = config.smtp_timeout or 10
...
def log_email_notification(
    ...
    recipients_value = ", ".join(rec for rec in recipients if rec)
    try:
        with db.engine.begin() as connection:
            connection.execute(
                EmailLog.__table__.insert(),
                {
                    "subject": subject,
                    "recipients": recipients_value or "-",
                    "body": body,
                    "success": success,
        ...
        app.logger.exception("E-posta logu kaydedilemedi")


with app.app_context():
    ensure_schema_upgrades()
    config = get_or_create_email_config()
...
def summarize_request_lines(request_lines: list["StockRequest"]) -> tuple[list[str], dict]:
    summaries: list[str] = []
    projects: set[str] = set()
    due_dates: list[str] = []
    for req in request_lines:
        product_name = req.product.name if req.product else f"Ürün #{req.product_id}"
        quantity_display = format_decimal_value(req.quantity, 0)
        unit_label = ""
        if req.product and req.product.unit:
            unit_label = req.product.unit.abbreviation or req.product.unit.name
        quantity_part = quantity_display
        if unit_label:
            quantity_part = f"{quantity_display} {unit_label}"
        project_name = req.project.name if req.project else "-"
        if project_name != "-":
            projects.add(project_name)
        if req.due_date:
            due_dates.append(req.due_date.strftime("%d.%m.%Y"))
            due_date_display = due_dates[-1]
        else:
            due_date_display = "-"
        summaries.append(
            f"- {product_name} | Miktar: {quantity_part} | Proje: {project_name} | Termin: {due_date_display}"
        )
    details = {
        "projects": projects,
        "due_dates": due_dates,
...
def build_request_email_body(
    requester: User | None,
    order_number: str,
    request_lines: list["StockRequest"],
    stage_label: str,
    link: str,
    notes: str | None,
) -> str:
    requester_name = requester.username if requester else "Bilinmeyen kullanıcı"
    lines, _ = summarize_request_lines(request_lines)
    parts = [
        f"{requester_name} tarafından yeni stok talebi oluşturuldu.",
        f"Sipariş numarası: {order_number}",
        f"Güncel aşama: {stage_label}",
        "",
        "Satırlar:",
        *lines,
    ]
    if notes:
        parts.extend(["", f"Talep Notu: {notes}"])
    parts.extend(["", f"Talebe erişmek için bağlantı: {link}"])
    return "\n".join(parts)


def render_request_email_content(
    request_lines: list["StockRequest"],
    requester: User | None,
    stage_label: str,
    order_number: str,
    ...
    if config is None:
        ...
        if config is None:
            config = get_or_create_email_config()
    summaries, details = summarize_request_lines(request_lines)
    lines_text = "\n".join(summaries)
    projects_text = ", ".join(sorted(details["projects"])) if details["projects"] else "-"
    due_dates_text = ", ".join(details["due_dates"]) if details["due_dates"] else "-"
    requester_name = requester.username if requester else "Bilinmeyen kullanıcı"
    ...
    context = {
        ...
        "due_dates_text": due_dates_text,
        "line_count": len(request_lines),
        "notes": notes or "-",
        "lines_text": lines_text or "-",
        "link": link,
    }
    subject_template = (config.auto_subject or DEFAULT_AUTO_EMAIL_SUBJECT) if config else DEFAULT_AUTO_EMAIL_SUBJECT
    body_template = (config.auto_body or DEFAULT_AUTO_EMAIL_BODY) if config else DEFAULT_AUTO_EMAIL_BODY
    try:
        subject = subject_template.format(**context)
    except Exception:
        subject = DEFAULT_AUTO_EMAIL_SUBJECT.format(**context)
    try:
        body = body_template.format(**context)
    except Exception:
        body = build_request_email_body(
            requester,
            order_number,
            request_lines,
            stage_label,
...
def send_request_notifications(order_number: str) -> None:
    requests = (
        StockRequest.query.options(
            joinedload(StockRequest.product).joinedload(Product.unit),
            joinedload(StockRequest.project).joinedload(Project.manager),
            joinedload(StockRequest.requester),
        )
        .filter(StockRequest.order_number == order_number)
        .order_by(StockRequest.id.asc())
        .all()
    )
    if not requests:
        return
    ...
    approval_link = url_for("stock_requests", _external=True)
    config = get_or_create_email_config()
    if config and config.auto_notify_enabled is False:
        return

    procurement_lines = [
        req for req in requests if req.status == "pending_procurement"
    ]
    if procurement_lines:
        recipients = {
            user.email
            for user in User.query.filter(
                User.role.in_(list(PROCUREMENT_APPROVER_ROLES))
            )
            if user.email
        }
        if recipients:
            subject, body = render_request_email_content(
                procurement_lines,
                requester,
                "1. onay (Satınalma Yöneticisi)",
                order_number,
                approval_link,
                notes,
    ...
    manager_groups: dict[User, list[StockRequest]] = {}
    for req in requests:
        if req.status != "pending_manager" or not req.manager or not req.manager.email:
            continue
        manager_groups.setdefault(req.manager, []).append(req)

    for manager, lines in manager_groups.items():
        subject, body = render_request_email_content(
            lines,
            requester,
            "2. onay (Proje Yöneticisi)",
            order_number,
            approval_link,
            notes,
            manager.username,
...
def email_settings():
    ...
    if request.method == "POST":
        ...
        mail_sender = request.form.get("mail_sender", "").strip() or None
        port_raw = request.form.get("smtp_port", "").strip()
        timeout_raw = request.form.get("smtp_timeout", "").strip()
        ...
        if port_raw:
            try:
                ...
                flash("Geçerli bir SMTP portu giriniz", "danger")
        ...
        if timeout_raw:
            try:
                ...
                flash("Geçerli bir zaman aşımı süresi giriniz", "danger")
                return redirect(url_for("email_settings"))
        else:
            timeout_value = None
        ...
        config.smtp_port = port_value
        config.smtp_timeout = timeout_value
        config.smtp_use_tls = use_tls
        record_user_activity("email_config_update", "E-posta ayarları güncellendi")
        db.session.commit()
        apply_email_config(config)
...
@app.route("/settings/email/client", methods=["GET", "POST"])
@login_required
@admin_required
def email_client():
    ...
    auto_form_data = {
        "enabled": True if config.auto_notify_enabled is None else bool(config.auto_notify_enabled),
        "subject": config.auto_subject or DEFAULT_AUTO_EMAIL_SUBJECT,
        "body": config.auto_body or DEFAULT_AUTO_EMAIL_BODY,
    }
    if request.method == "POST":
        form_type = request.form.get("form_type", "manual")
        if form_type == "auto":
            enabled = request.form.get("auto_notify_enabled") == "on"
            auto_subject = (request.form.get("auto_subject") or "").strip()
            auto_body = (request.form.get("auto_body") or "").strip()
            auto_form_data.update(
                {
                    "enabled": enabled,
            ...
            if not auto_subject:
                flash("Lütfen otomatik bildirim konusu girin", "danger")
            elif not auto_body:
                flash("Lütfen otomatik bildirim içeriğini girin", "danger")
            else:
                config.auto_notify_enabled = enabled
                config.auto_subject = auto_subject
                config.auto_body = auto_body
                db.session.commit()
                record_user_activity(
                    "email_auto_template_update",
                    "Otomatik talep bildirim şablonu güncellendi",
                )
                flash("Otomatik bildirim ayarları güncellendi", "success")
                return redirect(url_for("email_client"))
        else:
            subject = (request.form.get("subject") or "").strip()
            body = (request.form.get("body") or "").strip()
            selected_ids_raw = request.form.getlist("recipients")
            additional_raw = (request.form.get("additional_recipients") or "").strip()
            ...
            form_data.update(
                {
                    ...
                    "additional_recipients": additional_raw,
                }
            )

            if not subject:
                flash("Lütfen konu alanını doldurun", "danger")
            elif not body:
                flash("Lütfen mesaj içeriğini girin", "danger")
            else:
                ...
                if missing_users:
                    flash(
                        "E-posta adresi bulunmayan kullanıcılar: "
                        + ", ".join(sorted(set(missing_users))),
                        "warning",
                    )
                ...
                if not unique_recipients:
                    flash("En az bir geçerli alıcı belirtmelisiniz", "danger")
                else:
                    if send_email_notification(subject, body, unique_recipients):
                        record_user_activity(
                            "email_manual_send",
                            f"{len(unique_recipients)} alıcıya manuel e-posta gönderildi",
                        )
                        flash("E-posta başarıyla gönderildi", "success")
                        return redirect(url_for("email_client"))
                    flash(
                        "E-posta gönderimi başarısız oldu. Detaylar için e-posta loglarını kontrol edin.",
                        "danger",
                    )
    ...
    return render_template(
        ...
        auto_placeholders=AUTO_EMAIL_PLACEHOLDERS,
    )
...
def stock_requests():
    ...
    if request.method == "POST":
        ...
        db.session.commit()
        send_request_notifications(order_number)
        if auto_first_count:
