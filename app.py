...
def build_request_email_body(
    ...
    for req in request_lines:
        ...
        due_date = req.due_date.strftime("%d.%m.%Y") if req.due_date else "-"
        lines.append(
            f"- {product_name} | Miktar: {quantity_part} | Proje: {project_name} | Termin: {due_date}"
        )
    if notes:
        lines.extend(["", f"Talep Notu: {notes}"])
    lines.extend(["", f"Talebe erişmek için bağlantı: {link}"])
    return "\n".join(lines)
...
def send_request_notifications(order_number: str) -> None:
    ...
    if procurement_lines:
        ...
        if recipients:
            ...
            send_email_notification(
                f"[Stok Talebi] {order_number} için 1. onay bekliyor",
                body,
                sorted(recipients),
            )
    ...
    for manager, lines in manager_groups.items():
        ...
        send_email_notification(
            f"[Stok Talebi] {order_number} için 2. onay bekliyor",
            body,
            [manager.email],
        )
...
def email_client():
    ...
    return render_template("email_client.html", users=users, form_data=form_data)
